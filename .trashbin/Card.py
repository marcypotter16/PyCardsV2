...
class Card:
    def __init__(self, name: str, base_power: int, art_path: str, uid=None):
...
class CardController(GameObject):
    def __init__(self, game: Game):
        ...
        self.base_art = p.transform.scale(self.base_art, CARD_DIMENSIONS)
        self.back_art = p.image.load(
            CARD_BACK_PATH
        )  # is it a good idea to load the back of the card on every card even if not shown?
        self.back_art = p.transform.scale(self.back_art, CARD_DIMENSIONS)
    ...
    def from_card(self, card: Card):
        ...
        self.art = p.transform.scale(self.art, CARD_DIMENSIONS * CARD_ART_SIZE_RATIO)
    ...
    def scale(self, factor: float):
        ...
        self.base_art = p.transform.scale_by(self.base_art, factor)
        self.art_rect.scale_by_ip(factor)
        self.art = p.transform.scale_by(self.art, factor)
